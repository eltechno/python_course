...
for key, room_scenery_list in scenery.items():
    for scenery_item_list in room_scenery_list:
        checksum += (scenery_item_list[0] * check_counter
                     + scenery_item_list[1] * (check_counter + 1) 
                     + scenery_item_list[2] * (check_counter + 2))
        check_counter += 3
print(check_counter, "scenery data items")
##assert check_counter == 484, "Expected 484 scenery items"
##assert checksum == 1162728, "Error in scenery data"
